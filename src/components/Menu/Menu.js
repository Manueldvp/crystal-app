...
function Menu({ isOpen, isDesktop, handleOnClick }) {
  ...
  const showSubRoutes = (label) => {
    return allowedLabels.includes(label) && ((isDesktop && hoveredLabel === label) || (!isDesktop && clickedLabel === label));
  };
      ...
      <ul className={`flex flex-col font-medium p-4 mt-4 rounded-lg rtl:space-x-reverse md:flex-row md:mt-0 ${isDesktop === false ? 'shadow-lg' : ''} md:bg-white bg-transparent`}>
          {routes.map(({ label, route, icon, subRoutes, miniRoutes }) => (
            ...
            <li className='p-2  text-grey-cristal-400' key={route}
              ...
              <Link className={`flex items-center ${isDesktop ? '' : 'justify-between border-b mt-2 p-2'} hover:text-purple-secondary-500`} href={route} onClick={(e) => handleIconClick(e, label)}>
                {label}
                <span className={`${isDesktop ? 'ml-2' : 'ml-4'} `} onClick={handleOnClick} >{icon}</span>
              </Link>
              {showSubRoutes(label) && <SubMenu 
                isDesktop={isDesktop} 
                subRoutes={subRoutes} 
                icon={icon} 
