...
const QUIZZES = [
  { value: "pelvic-health", label: "Test Salud Pélvica", description: "Evalúa el estado general de tu piso pélvico" },
  { value: "postpartum", label: "Test Postparto", description: "Para mamás en etapa de recuperación" },
  { value: "endometriosis", label: "Test Endometriosis", description: "Identifica síntomas relacionados" },
]
...
const QUESTIONS = {
  ...
  "postpartum": [
    ...
    { 
      id: 5, 
      type: 'boolean',
      question: "¿Te gustaría retomar actividad física pero no sabes si es seguro?"
    },
  ],
  "endometriosis": [
    { 
...
function getDiagnosis(answers, quizType) {
  ...
  if (score >= 10) {
    return {
      ...
      message: "Basándome en tus respuestas, hay indicadores importantes que sugieren que podrías beneficiarte significativamente de la fisioterapia pélvica. Te recomiendo agendar una valoración para evaluar tu caso de forma personalizada.",
      cta: "Agenda tu valoración"
    }
  } else if (score >= 5) {
    return {
      level: 'medium',
      title: "Hay señales que vale la pena atender",
      message: "Tus respuestas indican algunos síntomas que, aunque quizás no sean severos ahora, podrían beneficiarse de atención preventiva. Una consulta puede ayudarte a entender mejor tu cuerpo y prevenir complicaciones.",
  ...
  return {
    ...
    message: "Según tus respuestas, no hay indicadores urgentes. Sin embargo, si en algún momento notas cambios o tienes dudas, no dudes en consultar. La prevención siempre es una buena inversión.",
    cta: "Conocer más"
  }
}
      ...
      {question.options.map((option, idx) => (
        ...
        >
          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
            ...
            {value === option && <div className="w-2.5 h-2.5 rounded-full bg-fuchsia-pink-500" />}
          </div>
      ...
      {question.options.map((option, idx) => (
        ...
        >
          <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${
            value.includes(option) ? 'border-fuchsia-pink-500 bg-fuchsia-pink-500' : 'border-gray-300'
          }`}>
            {value.includes(option) && (
...
function ContactForm({ data, onChange, onSubmit, isValid, isSubmitting, onBack }) {
  return (
    <div className="w-full max-w-lg">
      <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900 mb-3 leading-tight" style={{ fontFamily: 'Georgia, serif' }}>
        Tus resultados están casi listos.
      </h2>
      <p className="text-gray-600 mb-8">
        Comparte tus datos para personalizar tus recomendaciones.
      </p>
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">¿Cómo prefieres ser contactada?</label>
...
export default function QuizPage() {
  ...
  const [contactData, setContactData] = useState({ name: '', email: '', phone: '', contactMethod: 'WhatsApp' })
  ...
  const isContactValid = contactData.name.trim() !== '' && contactData.email.includes('@')
  ...
  const handleSubmitContact = async () => {
    ...
    try {
      await fetch('https://script.google.com/macros/s/AKfycbzj5gZCs7mw-T2CLBSpQlnb36asuhg8tW-Obw3MqE_-_P4yXG3D6xd-Y5Y7azx2AJfSeg/exec', {
        method: 'POST',
        mode: 'no-cors',
  ...
  const handleRestart = () => {
    ...
    setCurrentQuestion(0)
    setShowInfo(false)
    setContactData({ name: '', email: '', phone: '', contactMethod: 'WhatsApp' })
  }
    ...
    <div className="min-h-screen flex flex-col bg-fuchsia-pink-50">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-8 relative">
              ...
              {QUIZZES.map((quiz) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <span className="text-lg text-gray-900 font-semibold block">{quiz.label}</span>
        ...
        {step === 'questions' && !showInfo && currentQ && (
          <div className="w-full max-w-lg animate-fadeIn">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-8 leading-tight" style={{ fontFamily: 'Georgia, serif' }}>
              {currentQ.question}
            </h2>
        ...
        {step === 'results' && (
          <div className="w-full max-w-lg animate-fadeIn">
            <div className={`p-1 rounded-2xl mb-6 ${
              diagnosis.level === 'high' ? 'bg-gradient-to-r from-fuchsia-pink-400 to-fuchsia-pink-600' :
              diagnosis.level === 'medium' ? 'bg-gradient-to-r from-fuchsia-pink-300 to-fuchsia-pink-400' :
              'bg-gradient-to-r from-gray-200 to-gray-300'
            }`}>
              <div className="bg-white rounded-xl p-6">
                <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4" style={{ fontFamily: 'Georgia, serif' }}>
                  {contactData.name}, {diagnosis.title.toLowerCase()}
                </h2>
            ...
            <div className="flex flex-col gap-4">
              <a
                href={`https://api.whatsapp.com/send?phone=5212224237337&text=Hola%20Cristal%20%F0%9F%91%8B%20Soy%20${encodeURIComponent(contactData.name)}%20y%20realicé%20el%20cuestionario.%20Me%20gustaría%20agendar%20una%20valoración.`}
                target="_blank"
